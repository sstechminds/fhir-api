...
MAINTAINER Ravindar Vangala<ravi.v@flowsigma.com>

ENV SPRING_PROFILE_NAME=""
ENV SPRING_CONFIG_NAME="ewocsfhirapi"
ENV JAVA_OPTS="-Dspring.profiles.active=$SPRING_PROFILE_NAME -Dspring.config.name=$SPRING_CONFIG_NAME"

ENV APP_FILE ewocsfhirapi.jar
ENV APP_HOME /app

RUN mkdir -p $APP_HOME

COPY build/libs/ewocs_fhir_api-*.jar $APP_HOME/$APP_FILE
...
# docker build --tag flowsigma/ewocsfhirapi:latest .
# docker run ---env SPRING_CONFIG_NAME=ewocsfhirapi -publish 8009:8009 --name ewocsfhirapi flowsigma/ewocsfhirapi:latest
